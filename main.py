...
player3 = Person("Robot:", 3750, 155, 288, 34, player_magic, player_items)
...
while running:
    ...
    for enemy in enemies:
        ...
        if enemy_choice == 0:
            ...
            print(enemy.name.replace(" ", "") + " attacks " + players[target].name.replace(" ", "").replace(":", "") + " for", enemy_dmg, "points.")
            ...
            if spell.type == "white":
                ...
                print(bcolors.OKBLUE + "\n" + enemy.name.replace(" ", "") + "'s " + spell.name + " deals", str(magic_dmg), "points of damage to " + players[target].name.replace(" ", "").replace(":", "")+ "." + bcolors.ENDC)
